...
class BackendConfig:
    ...
    def _validate(self):
        ...
        if not re.match(r'^[a-zA-Z0-9_-]+$', self.name):
            raise ValueError("Backend name can only contain letters, numbers, hyphens, and underscores")
        ...
        if parsed.scheme not in supported_schemes:
            logger.warning(f"Unsupported scheme '{parsed.scheme}' for backend '{self.name}'")
...
class BackendManager:
    ...
    def register_backend(self, name: str, url: str, description: str = "",
                        readonly: bool = False, timeout: int = 30,
                        retry_attempts: int = 3, set_as_default: bool = False,
                        validate_connection: bool = True) -> bool:
        """
        ...
        config = BackendConfig(name, url, description, readonly, timeout, retry_attempts)
        ...
        try:
            ...
            if validate_connection:
                raise ConnectionError(f"Backend '{name}' connection failed: {e}")
            return False
    ...
    def get_backend(self, name: Optional[str] = None) -> DAL:
        """
    ...
    def remove_backend(self, name: str, force: bool = False):
        ...
        if name == self.default_backend and not force:
            raise ValueError(f"Cannot remove default backend '{name}' without force=True")
    ...
    def check_backend_health(self, name: Optional[str] = None) -> Dict[str, bool]:
        """
    ...
    def get_backend_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "healthy_backends": sum(1 for status in self._health_status.values() if status),
            "readonly_backends": sum(1 for config in self.configs.values() if config.readonly),
            "backend_schemes": list(set(urlparse(config.url).scheme for config in self.configs.values()))
        }
